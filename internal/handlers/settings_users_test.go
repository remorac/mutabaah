...
func TestUserDetailRendersDataActionsAndReadOnlyPeriods(t *testing.T) {
	...
	view := userDetailView{
		BaseView:          BaseView{Title: "Fatimah", UserName: "Admin", UserRole: string(repository.UsersRoleAdmin), CSRFToken: "csrf-token", AvatarPath: "/static/avatars/thumb_admin.jpg"},
		ID:                2,
		Email:             "fatimah@example.com",
		Name:              "Fatimah",
		Role:              string(repository.UsersRoleUser),
	...
	for _, want := range []string{
		...
		"fatimah@example.com",
		`src="/static/avatars/thumb_2_avatar.jpg"`,
		`src="/static/avatars/thumb_admin.jpg"`,
