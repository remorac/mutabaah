...
// given dates. Every submitted date must fall inside allowedDates — anything
// else is rejected as a validation error so admins can't accidentally erase
// older history. An empty dates slice is a no-op.
func (s *UserAdminService) ResetCompletions(ctx context.Context, userID int64, dates, allowedDates []time.Time) error {
